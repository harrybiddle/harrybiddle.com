    ...
    onMount(async () => {
        ...
        budgets = await Promise.all(
            months.map(month => {
                ...
                const response = ynab(ynabToken, `budgets/${budgetId}/months/${monthString}`, {})
                return response.then(r => ({...r, monthObject: month}))
            })
        );
    });
