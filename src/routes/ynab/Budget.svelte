<script>
    ...
    const lines = saturdays.map((d) => d.date() / numberDaysInMonth);
    ...
    const parsedCategories = budget.month.categories
        ...
        .map(e => ({...e, activity: e.group == "One-Off" ? e.budgeted : e.activity}))
         // also ignore actiity of Scheduled; unless it exceeds budget
        .map(e => ({...e, activity: e.group == "Scheduled" ? Math.max(e.activity, e.budgeted): e.activity}))
    ...
    const final = d3
        ...
        .reduce(
          (accumulator, [group, items], index, array) => {
            ...
            const totalActivity = sumActivity(items);
            const totalBudgeted = sumBudgeted(items);
            const bars = ["Scheduled", "One-Off"].includes(group) ? false : true;
            return [
              ...accumulator,
              ...
              ...(group == "Regular"
                ? sortedItems.map((d) => ({
                    level: 0,
                    name: d.category,
                    bars: d.category == "Uncategorized" ? false : bars,
                    budgeted: d.budgeted,
                    activity: d.activity
                  }))
                : [])
...
<table class="sparkbars">
    {#each final as c}
        ...
        {@const y = c.budgeted * currentDay / numberDaysInMonth - c.activity }
            <tr style="
