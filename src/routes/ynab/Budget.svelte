<script>
    ...
    const parsedCategories = budget.month.categories.map(
        d => ({
...
<table>
    {#each parsedCategories as c}
        {#if (c.activity || c.budgeted) }
            <tr>
                <td>{c.category}</td>
                <td>{formatZero(c.budgeted)}</td>
                ...
                <td>
                    {formatZero((c.budgeted * currentDay) / numberDaysInMonth - c.activity)}
                </td>
            </tr>
        {/if}
    {/each}
